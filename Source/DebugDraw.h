...
void DebugDraw(const LineSegment& segment, Color color = White, bool is_arrow = false, const float4x4& transform = float4x4::identity, const float& line_width = 1.0f);
...
void DebugDrawBox(const float3* corners, Color color,bool debug = false);
